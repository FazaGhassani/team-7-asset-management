...
export class WarehouseAddComponent implements OnInit {
  ...
  warehouse: Warehouse[] = []
  modelWarehouse = new WarehouseModel('')
  submitted = false
  ...
  addWarehouse(name: string = this.modelWarehouse.name): void {
    name = name.trim()
    if (!name) { return }
    this.warehouseService.addWarehouse({ name } as Warehouse).subscribe(wrh => { this.warehouse.push(wrh) })
    this.warehouse = []
  }
  goBack(): void {
    this.location.back()
